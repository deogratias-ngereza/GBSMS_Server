<?php
// Operation mob api middleware

/* 
	CHECK POSTED LOGIN
*/
$op_mob_check_login_params = function($request,$response,$next){
	//$response->getBody()->write("Before - ");
	$response = $next($request,$response);
	//$response->getBody()->write(" - After");
	return $response;

	//return $response->withJson(["msg_data" => "Invalid parameters","msq_status" => "ERROR"])->withStatus(401);
};
