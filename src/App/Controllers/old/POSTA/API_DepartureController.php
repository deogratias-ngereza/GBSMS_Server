...
class API_DepartureController extends BaseController{


	//custom search details
	public function search_departures_from_dates_center_and_department($req,$res,$args){
		$io_mode = $this->helperController->get_io_with_connector_for_arrival_departure_from_io_mode($args['io_mode']);

		if($args['status'] == "ALL"){
			$IODeparture = IODeparture::where('date_exp_departure','>=',$args['start_dt'])
		 				->where('date_exp_departure','<=',$args['end_dt'])
		 				->where('io_mode','=',$args['io_mode'])
		 				->where('id_center_from','=',$args['c_id'])
	...
	/*
		Delete the io departure with the respected io arrivals for it
	*/
	public function delete($req,$res,$args){
		...
		//delete entry from associated arrivals
		$arrivals = IOArrival::where('io_departure_ref','=',$args['id'])->delete();
	...
	/*
		update the info in io departure with the respected io arrivals
		- date and time
	*/
	public function update_sch_period($req,$res,$args){
		...
		$inputs = [
			"date_exp_departure" => $updates['date_exp_departure'],
			"time_exp_departure" => $updates['time_exp_departure']
		];
		$update_status = IODeparture::where('id',$args['id'])
						->update($inputs);
		$results = IODeparture::where('id',$args['id'])->first();

		$arrivals = IOArrival::where('io_departure_ref','=',$args['id'])
		->update(["date_exp_arrival"=>$updates['date_exp_arrival']]);
